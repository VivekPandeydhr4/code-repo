...
int is_isomorphic(node* T1, node* T2)
{
  if(!T1 && !T2) return 1;
  ...
  else if (T1 && T2)
    {
      ...
      if ((is_isomorphic(T1->left, T2->left) && is_isomorphic(T1->right, T2->right))
          || (is_isomorphic(T1->left, T2->right) && is_isomorphic(T1->right, T2->left)))
	return 1;
      else
	return 0;
    }
  ...
  if (s<=e)
    {
      ...
      else if (k < arr[mid]) binary_search(arr, s, mid-1, k);
      else return binary_search(arr, mid-1, e, k);
    }
  else return -1;
}

void print_postorder (int *p, int *In, int ps, int pe, int Is, int Ie)
{
  int i;
  if(ps <= pe ^ Is<=Ie) printf("Invalid Input\n");
  else if (ps<=pe && Is<=Ie)
    {
      i = binary_search(In, Is, Ie, p[ps]);
      if (i==-1) printf("Invalid Input\n");
      else
	{
	  print_postorder (p, In, ps+1, ps+i-Is, Is, i-1);
	  print_postorder (p, In, ps+i-Is+1, pe, i+1, Ie);
	  printf(" %d", p[ps]);
	}
    }
}

void postorder_with_In_pre(int *pre, int *in, int n)
{
  if (n>0)
    {
      print_postorder(pre, in, 0, n-1, 0, n-1);
    }
}
...
int main()
{
  ...
  int pre[] = {15, 10, 5, 4, 6, 8, 13, 11, 18, 16, 17, 19, 22, 20, 21};
  int size = sizeof(pre)/sizeof(int);

  postorder_with_In_pre(pre, In, size);

  /*
  node* root1 = make_tree();
  root1->left->left->data = 13;

  printf("Is Isomorphic = %s\n", is_isomorphic(root, root1)? "Yes" : "No");
